// frontend/src/components/widgets/UsersTableWidget.jsx
...
export default function UsersTableWidget({ config }) {
  const { title = 'Users', role = 'all', pageSize = 10 } = config;
  const [page, setPage] = useState(1);
  
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['table', 'users', page, role, pageSize],
          ...
          <tbody className="divide-y divide-gray-200">
            {data.rows.map((row) => (
              <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-sm"><RoleBadge role={row.role} /></td>
                <td className="px-4 py-3 text-sm"><StatusBadge status={row.status} /></td>
                <td className="px-4 py-3 text-sm text-gray-600">{formatDate(row.createdAt, 'short')}</td>
              </tr>
            ))}
          </tbody>
        </table>
